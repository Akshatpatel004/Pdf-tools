...
const Login = () => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
            if (currentUser) navigate("/"); // Redirect to home if already logged in
        });
        return () => unsubscribe();
    }, [navigate]);
    ...
    return (
        <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#0F172A] font-['Montserrat']">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden">
                <div className="absolute -top-[120px] -left-[120px] w-[400px] h-[400px] rounded-full bg-sky-500 blur-[120px] opacity-60"></div>
                <div className="absolute -bottom-[150px] -right-[150px] w-[450px] h-[450px] rounded-full bg-teal-500 blur-[120px] opacity-60"></div>
            </div>
            ...
            <div className="relative z-10 w-[850px] max-w-[95%] min-h-[550px] bg-[#0F172A]/75 backdrop-blur-3xl border border-white/15 rounded-[30px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
                ...
                <div className={`absolute top-0 h-full w-full md:w-1/2 flex items-center justify-center p-8 md:p-14 transition-all duration-600 ease-in-out ${isSignup ? "translate-x-0 md:translate-x-full opacity-100 z-50" : "opacity-0 z-10"}`}>
                    <form onSubmit={handleSignup} className="w-full flex flex-col">
                        ...
                        <input className="bg-white/5 border border-white/15 p-3.5 rounded-xl mb-4 text-white outline-none focus:border-sky-500 transition-all" type="password" placeholder="Password" value={sPassword} onChange={e => setSPassword(e.target.value)} required />
                        <button type="submit" className="py-3 mt-2 rounded-xl bg-gradient-to-br from-sky-500 to-teal-500 text-white font-semibold hover:-translate-y-0.5 transition-all">Create Account</button>
                ...
                <div className={`absolute top-0 h-full w-full md:w-1/2 flex items-center justify-center p-8 md:p-14 transition-all duration-600 ease-in-out ${isSignup ? "translate-x-0 md:translate-x-full opacity-0 z-10" : "translate-x-0 opacity-100 z-20"}`}>
                    <form onSubmit={handleLogin} className="w-full flex flex-col">
                        ...
                        <button type="button" onClick={handleGoogle} className="mb-4 py-3 bg-white/10 border border-white/15 rounded-xl text-white font-semibold hover:bg-white/15 transition-all">Sign in with Google</button>
                        <input className="bg-white/5 border border-white/15 p-3.5 rounded-xl mb-4 text-white outline-none focus:border-sky-500 transition-all" type="email" placeholder="Email" value={lEmail} onChange={e => setLEmail(e.target.value)} required />
                        <input className="bg-white/5 border border-white/15 p-3.5 rounded-xl mb-4 text-white outline-none focus:border-sky-500 transition-all" type="password" placeholder="Password" value={lPassword} onChange={e => setLPassword(e.target.value)} required />
                        <button type="submit" className="py-3 mt-2 rounded-xl bg-gradient-to-br from-sky-500 to-teal-500 text-white font-semibold hover:-translate-y-0.5 transition-all">Sign In</button>
                ...
                <div className={`hidden md:block absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-all duration-600 ease-in-out z-100 ${isSignup ? "-translate-x-full rounded-r-[150px]" : "rounded-l-[150px]"}`}>
                    <div className={`relative -left-full h-full w-[200%] bg-gradient-to-br from-sky-500 to-teal-500 transition-all duration-600 ease-in-out ${isSignup ? "translate-x-1/2" : "translate-x-0"}`}>
