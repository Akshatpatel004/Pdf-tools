document.addEventListener("DOMContentLoaded", function () {
	...
	let totalFiles = 0;
	let selectedFiles = [];
  const ispptxfiletype='application/vnd.openxmlformats-officedocument.presentationml.presentation';

	function uploadFile(file) {
		let listItem = document.createElement("li");
		let pptxicon = document.createElement("img");
		pptxicon.classList.add("fileicon");
		let iconsrc = (pptxicon.src = "../icons/ppt.png");

		let fileName = file.name;
		let fileSize = (file.size / 1024).toFixed(1);
		listItem.innerHTML = `<img src = "${iconsrc}"style="height:10px;width:10px;"><p>${fileName}</p><span>${fileSize}KB</span>`;
		if (fileSize >= 1024) {
	...
	function handlefiles(files, dragfiles) {
		const validFiles = Array.from(files).filter(file => file.type === ispptxfiletype);
		...
		if (!validFiles) {
			alert("Please upload only PPT file");
		}
		...
		if (totalFiles > 0) {
			...
			totalFiles = 0;
			alert("Please upload atleast 1 PPT file");
		}
	}
	...
	try {
		...
		dropArea.addEventListener("drop", (e) => {
			...
			let pptx = Array.from(dragfiles).filter(
				(f) => f.type === ispptxfiletype
			);
			console.log(pptx, "?:>:?>:");
	...
	submitBtn.addEventListener("click", async function (e) {
		if (selectedFiles.length < 0) {
			alert("Please upload at least one PPT files.");
			return;
		}
		e.preventDefault();
		const formData = new FormData();
		selectedFiles.forEach((file) => {
			formData.append("pptx_pdf", file);
		});
		try {
			const response = await fetch("/pptxtopdf", {
				method: "POST",
				body: formData,
			});
			...
			if (response.ok) {
				...
				if (selectedFiles.length==1) {
					a.download = `PPTX to pdf ${Date.now()}.pdf`;
				} else if(selectedFiles.length>1){
					a.download = `PPTX_to_pdf convert ${Date.now()}.zip`;
				}
