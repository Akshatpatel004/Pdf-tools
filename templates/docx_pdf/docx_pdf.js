document.addEventListener("DOMContentLoaded", function () {
	...
	let totalFiles = 0;
	let selectedFiles = [];
  const isdocxfiletype='application/vnd.openxmlformats-officedocument.wordprocessingml.document';

	function uploadFile(file) {
		let listItem = document.createElement("li");
		let docxicon = document.createElement("img");
		let iconsrc = (docxicon.src = "../icons/doc.png");
	...
	function handlefiles(files, dragfiles) {
		...
		if (!validFiles) {
			alert("Please upload only DOCX file");
		}
		...
		if (totalFiles > 0) {
			...
			totalFiles = 0;
			alert("Please upload atleast 1 DOCX file");
		}
	}
	...
	submitBtn.addEventListener("click", async function (e) {
		if (selectedFiles.length < 0) {
			alert("Please upload at least one DOCX files.");
			return;
		}
		...
		selectedFiles.forEach((file) => {
			formData.append("docx_pdf", file);
		});
		// formData.append("pdflen", selectedFiles.length);

		try {
			const response = await fetch("/docxtopdf", {
				method: "POST",
				body: formData,
			});
			...
			if (response.ok) {
				...
				if (selectedFiles.length==1) {
					a.download = `DOCX to pdf ${Date.now()}.pdf`;
				} else if(selectedFiles.length>1){
					a.download = `DOCX_to_pdf convert ${Date.now()}.zip`;
				}
